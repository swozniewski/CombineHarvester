...
for MODE in ['prefit']:
    for CHN in ['mt']: #['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            LABEL = "%s %s" % (CHN_DICT[CHN], CAT_DICT[CAT])
            YMIN = "%s" % RANGE_DICT[CHN]
            PAD = "%s" % PAD_DICT[CHN]
            os.system(('python CombineHarvester/HIG16037/scripts/postFitPlot.py' \
                  ' --file=shapes_300916.root --ratio --extra_pad="%(PAD)s" --mPhi=3200 ' \
...
for MODE in ['prefit']:
    for CHN in ['mt']: #['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            LABEL = "%s %s" % (CHN_DICT[CHN], CAT_DICT[CAT])
            os.system(('python CombineHarvester/HIG16037/scripts/postFitPlot.py' \
                  ' --file=shapes_300916.root --ratio --extra_pad=0.6 --mPhi=3200 --no_signal' \
